...
[ -b "$root_part" ] || _pixie_die "root_part ${root_part} is not a block device"
...
# Best-effort status POST so pixie's timeline reflects "ramboot up"
# before pivot. Silent on failure.
server="$(_pixie_getarg server)"
mac="$(_pixie_getarg mac)"
