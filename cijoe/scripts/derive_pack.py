...
def main(args, cijoe):
    ...
    # NOSI_DERIVE_VARIANT (set by build.yml's per-variant derive job) selects a
    # single shape, so each derive runs in its own GHA job with its own cijoe
    # report. cijoe does not forward CLI flags to a script, hence the env var.
    # Unset means build every derive in the base's list (the local / serial path).
    only = os.environ.get("NOSI_DERIVE_VARIANT", "").strip()
    if only:
        derives = [e for e in derives if e["variant"] == only]
        if not derives:
            log.error(f"NOSI_DERIVE_VARIANT {only!r} is not a derive of '{image_name}'")
            return errno.EINVAL
